...
public static class AnthropicClientSettingsLoader
{
        ...
        List<string> diagnostics)
    {
        ...
        if (TryGetPropertyCaseInsensitive(root, "Aexon", out var claudeSharp))
        {
            if (claudeSharp.ValueKind != JsonValueKind.Object)
            {
                diagnostics.Add("Anthropic config: Aexon section must be a JSON object.");
            }
            else if (TryGetPropertyCaseInsensitive(claudeSharp, "Anthropic", out var nestedAnthropic))
            {
                ...
                {
                    diagnostics.Add("Anthropic config: Aexon.Anthropic must be a JSON object.");
                    return null;
                }
