...
/// <summary>
/// Stores a deleted-message placeholder.
/// </summary>
public record TombstoneMessage : SystemMessage
{
    public TombstoneMessage()
    {
        Subtype = SystemMessageSubtypes.Tombstone;
    }

    public string? DeletedMessageId { get; init; }
    public string? DeletedMessageType { get; init; }
    public string? Reason { get; init; }
}
...
internal static class StructuredSystemMessageCodec
{
        ...
        JsonElement? payload)
    {
        var restored = subtype switch
        {
            ...
            SystemMessageSubtypes.Tombstone => Deserialize<TombstoneMessage>(payload),
            _ => null,
        };
